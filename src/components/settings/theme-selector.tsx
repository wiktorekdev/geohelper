...
export function ThemeSelector() {
  ...
  const items = useMemo(
    () => themes.map((th) => ({ value: `${th.name} ${th.id}`, theme: th })),
    [themes]
  )
      ...
      <PopoverContent align="start" sideOffset={6} className="w-[var(--anchor-width)] p-0">
        <Command items={items}>
          ...
          <CommandList id="theme-listbox">
            {(item: { value: string; theme: Theme }) => (
              <CommandItem
                key={item.theme.id}
                value={item.value}
                onClick={() => {
                  setActive(item.theme.id)
                  setOpen(false)
                }}
                className="gap-2"
              >
                <ThemeSwatch theme={item.theme} />
                <span className="truncate text-xs min-w-0 flex-1">{item.theme.name}</span>
                {!item.theme.builtin && (
                ...
                <Check
                  className={cn(
                    ...
                    activeId === item.theme.id ? "opacity-100 text-brand" : "opacity-0"
                  )}
                />
              </CommandItem>
            )}
