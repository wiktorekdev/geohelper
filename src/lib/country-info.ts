...
export function fetchCountryDetails(
  ...
  const promise = (async () => {
    try {
      const res = await fetch(`https://restcountries.com/v3.1/alpha/${cc}?fields=${FIELDS}`, {
      ...
      const languages = entry.languages ? (Object.values(entry.languages) as string[]) : undefined
      ...
      const idd = entry.idd
      const callingCode =
        idd?.root && idd.suffixes?.length === 1 ? `${idd.root}${idd.suffixes[0]}` : idd?.root
