...
export type GoogleApiKeyValidation = { ok: true; message: string } | { ok: false; message: string }
...
export async function validateGoogleApiKey(
  ...
  if (!res.ok)
    return { ok: false, message: t("validation.httpError", { status: res.status.toString() }) }
  ...
  switch (data.status) {
    ...
    default:
      return {
        ...
        message:
          data.error_message ||
          t("validation.unknownStatus", { status: data.status || "an unknown status" }),
      }
  }
}
