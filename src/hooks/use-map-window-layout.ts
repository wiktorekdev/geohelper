    ...
    void (async () => {
      try {
        ...
        if (!mapVisible) {
          ...
          const targetWidth = storedWidth ? Math.max(MIN_WITH_MAP_W, storedWidth) : MIN_WITH_MAP_W
          ...
          if (needsResize) {
            await win.setSize(
              new LogicalSize(Math.max(targetWidth, wLog), Math.max(MIN_WITH_MAP_H, hLog))
            )
          }
        }
      } catch {
